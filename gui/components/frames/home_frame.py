...
class HomeFrame(customtkinter.CTkFrame):
    def __init__(self, master, app, dbAPI):
        ...
        self.create_order_button = customtkinter.CTkButton(
            self, text="Create Order", height=60, width=200, font=("", 24), corner_radius=10, command=self.create_order)
        ...
        self.add_user_button = customtkinter.CTkButton(
            self, text="Add user", command=self.add_user)
        self.add_user_button.grid(row=2, column=1, pady=20, sticky="e")
        self.remove_user_button = customtkinter.CTkButton(
            self, text="Remove user", command=self.remove_user)
    ...
    def open_popup_form(self, button_type):
        if button_type == "order":
            form = DynamicPopup(self.app, "Create Order",
                                [], dbAPI=self.dbAPI, create_order=True)
        elif button_type == "newuser":
            form = DynamicPopup(self.app, "Create User", [
                                "Username:", "Permissions:", "Password:", "Your password:"], dbAPI=self.dbAPI)
        elif button_type == "deleteuser":
            form = DynamicPopup(self.app, "Remove User", [],
                                dbAPI=self.dbAPI, remove_users=True)
        ...
        if button_type == "newuser":
            ...
            res = self.dbAPI.create_user(
                username, curr_password, password, permissions)
            if res is None:
