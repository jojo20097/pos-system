...
class DynamicPopup(customtkinter.CTkToplevel):
    def __init__(self, parent, title, prompts, dbAPI, create_order: bool = False, create_menu: bool = False, remove_users: bool = False):
        ...
        for i, prompt in enumerate(prompts):
            ...
            label.grid(row=i*2+1, column=0, padx=100 if not create_menu else 20,
                       pady=0 if not create_menu else (20 if i == 0 else 0, 0), columnspan=3 if not create_menu else 1)
            entry = customtkinter.CTkEntry(self, width=250)
            entry.grid(row=i*2+2, column=0, padx=100 if not create_menu else 20,
                       pady=0 if not create_menu else 0, columnspan=3 if not create_menu else 1)
            self.input_entries.append(entry)

        if create_menu:
            self.added_items_frame = TableFrame(self, self.app, ["Name", "Amount"], [
            ], False, False, False, False, False)
            self.added_items_frame.grid(
                row=5, column=0, columnspan=1, rowspan=1, sticky="nesw", padx=20, pady=20)
            self.add_item_button = customtkinter.CTkButton(
                self, text="Add Item", command=self.add_to_menu)
            self.add_item_button.grid(row=1, column=3, rowspan=6, padx=20)

        if create_order:
            self.added_items_frame = TableFrame(
                self, self.app, ["Name", "Amount", "Cost"], [], False, False, search=False)
            self.added_items_frame.grid(
                row=0, column=0, columnspan=2, sticky="nesw", padx=20, pady=20)
            self.add_item_button = customtkinter.CTkButton(
                self, text="Add Item", command=self.add_to_order)
        ...
        if remove_users:
            self.get_all_users()
            self.users_frame = TableFrame(self, self.app, [
                                          "ID", "Name", "Permissions"], self.ui_items, False, True, False, False, False)
            self.users_frame.grid(
                row=0, column=0, columnspan=3, sticky="nesw", padx=20, pady=20)

        # Add Confirm button
        ok_button = customtkinter.CTkButton(
            self, text="Confirm", command=self.get_input_values)
        ok_button.grid(row=2*len(prompts)+2, column=0, columnspan=3,
                       padx=100 if not create_menu else 20, pady=20)

        # Add Cancel button
        cancel_button = customtkinter.CTkButton(
            self, text="Cancel", command=self.close)
        cancel_button.grid(row=2*len(prompts)+2, column=2,
                           padx=20, pady=20, sticky="we")
