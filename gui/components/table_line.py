...
class TableLine(customtkinter.CTkFrame):
    ...
    def modify_table(self):
        if self.parent.column_names == ["Id", "Name", "Price", "UOM", "Amount"]: # Modify inventory item
            ...
            res = self.dbAPI.add_inventory_item_amount(item.inventory_item, int(difference)) if difference > 0 else self.dbAPI.sub_inventory_item_amount(item.inventory_item, -int(difference))
            if res is None:
